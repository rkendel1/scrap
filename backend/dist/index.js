...
// Update user profile
app.patch('/api/profile', authService.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
        ...
        if (email && !profile_service_1.profileService.isValidEmail(email)) {
            return res.status(400).json({ error: 'Invalid email format' });
        }
        const updatedUser = await profile_service_1.profileService.updateProfile(req.user.id, {
            first_name,
            last_name,
            email,
            profile_picture_url,
...
// Update password
app.patch('/api/profile/password', authService.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Authentication required' });
        }
        ...
        const passwordValidation = profile_service_1.profileService.isValidPassword(new_password);
        ...
        await profile_service_1.profileService.updatePassword(req.user.id, {
            current_password,
            new_password,
        });
        res.json({
...
app.post('/api/profile/forgot-password', async (req, res) => {
    try {
        ...
        if (!profile_service_1.profileService.isValidEmail(email)) {
            return res.status(400).json({ error: 'Invalid email format' });
        }
        const token = await profile_service_1.profileService.generatePasswordResetToken(email);
        if (token) {
            await profile_service_1.profileService.sendPasswordResetEmail(email, token);
        }
...
app.post('/api/profile/reset-password', async (req, res) => {
    try {
        ...
        await profile_service_1.profileService.resetPasswordWithToken(token, new_password);
        res.json({
            success: true,
...
app.get('/api/profile/notifications', authService.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Authentication required' });
        }
        const preferences = await profile_service_1.profileService.getNotificationPreferences(req.user.id);
        res.json({
            success: true,
            preferences,
        });
...
app.patch('/api/profile/notifications', authService.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Authentication required' });
        }
        ...
        await profile_service_1.profileService.updateNotificationPreferences(req.user.id, preferences);
        res.json({
            success: true,
            preferences,
        });
...
// Deactivate account
app.post('/api/profile/deactivate', authService.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Authentication required' });
        }
        const { reason } = req.body;
        await profile_service_1.profileService.deactivateAccount(req.user.id, reason);
        res.json({
            success: true,
...
app.get('/api/profile/notifications/events', authService.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Authentication required' });
        }
        ...
        const notifications = await profile_service_1.profileService.getUserNotifications(req.user.id, limit, offset);
        res.json({
            success: true,
            notifications,
        });
...
app.patch('/api/profile/notifications/events/:id/read', authService.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Authentication required' });
        }
        ...
        const success = await profile_service_1.profileService.markNotificationAsRead(req.user.id, notificationId);
        if (success) {
            res.json({
                success: true,
...
// Duplicate form
app.post('/api/forms/:id/duplicate', authService.authenticateToken, async (req, res) => {
    try {
        const formId = parseInt(req.params.id);
        if (isNaN(formId)) {
...
// Delete form
app.delete('/api/forms/:id', authService.authenticateToken, async (req, res) => {
    try {
        const formId = parseInt(req.params.id);
        if (isNaN(formId)) {
            return res.status(400).json({ success: false, message: 'Invalid form ID' });
...
// Update form tags
app.patch('/api/forms/:id/tags', authService.authenticateToken, async (req, res) => {
    try {
        const formId = parseInt(req.params.id);
        const { tags } = req.body;
        if (isNaN(formId)) {
...
// Get billing history
app.get('/api/subscription/billing-history', authService.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Authentication required' });
        }
      ...
      LIMIT $2 OFFSET $3
    `;
        const result = await database_1.default.query(query, [req.user.id, limit, offset]);
        // Format the response
...
app.get('/api/subscription/upcoming-billing', authService.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Authentication required' });
        }
