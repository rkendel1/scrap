...
try {
  ...
  subscriptionMiddleware = new SubscriptionMiddleware();
...
app.post('/api/forms/flow/form-type-selected', authService.optionalAuth, async (req: AuthRequest, res) => {
  try {
...
app.post('/api/forms/flow/generate', authService.optionalAuth, async (req: AuthRequest, res) => {
  try {
    const { 
      extractedRecordId, 
      formPurpose, 
    ...
    const { form: generatedForm, flowState } = await llmService.generateFormWithFlow(
      websiteData, 
      formPurpose, 
      userRole, 
      !!req.user
    ...
    const embedResult = llmService.generateEmbedCodeWithExpiration(
      form.id, 
      userRole, 
      !!req.user
    );
...
app.get('/api/forms/:id/embed-status', authService.optionalAuth, async (req: AuthRequest, res) => {
  try {
    const formId = parseInt(req.params.id);
    if (isNaN(formId)) {
      return res.status(400).json({ error: 'Invalid form ID' });
    ...
    const form = req.user ? 
      ...
      await saasService.getFormByEmbedCode(req.query.embedCode as string);
...
app.get('/api/subscription/current', authService.authenticateToken, async (req: AuthRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Authentication required' });
    }
...
app.post('/api/subscription/checkout', authService.authenticateToken, async (req: AuthRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Authentication required' });
    }
    ...
    const customerId = await stripeService.createOrGetCustomer(
      req.user.id, 
      req.user.email, 
      req.user.first_name ? `${req.user.first_name} ${req.user.last_name || ''}`.trim() : undefined
    );
...
app.post('/api/subscription/billing-portal', authService.authenticateToken, async (req: AuthRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Authentication required' });
    }
