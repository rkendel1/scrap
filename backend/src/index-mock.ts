...
import { createMockDatabaseWithSampleData } from './sample-data';
...
const dbService = createMockDatabaseWithSampleData();
...
app.get('/health', async (req, res) => {
  try {
    ...
    res.json({ 
      ...
      database: dbHealthy ? 'connected (mock)' : 'disconnected'
    });
  } catch (error) {
    res.status(500).json({ 
      status: 'unhealthy', 
